...
class PullCommand {
  ...
  async run(packageSpec, options = {}) {
    console.log(chalk.magenta(`📥 Pulling Vibe Kit Configuration: ${packageSpec}\n`));
    ...
    const backupDir = options.backup ? `.vibe-kit.backup.${Date.now()}` : null;
    
    if (await fs.pathExists('.vibe-kit') && !options.force) {
      if (backupDir) {
        await fs.copy('.vibe-kit', backupDir);
        console.log(chalk.green(`✅ Backed up existing .vibe-kit to ${backupDir}`));
      } else {
    ...
    // Copy package files
    await this.copyPackageFiles(packageDir, '.vibe-kit', options);
  ...
  async promptOverwrite() {
    ...
    return new Promise((resolve) => {
      readline.question(chalk.yellow('⚠️  .vibe-kit already exists. Overwrite? (y/N): '), (answer) => {
        readline.close();
