...
class RunCommand {
  ...
  async run(workflowName, options = {}) {
    ...
    // Find workflow file
    const possiblePaths = [
      `.vibe-kit/instructions/core/${workflowName}.md`,
      `.vibe-kit/instructions/${workflowName}.md`,
      `.vibe-kit/commands/${workflowName}.md`
    ];
  ...
  async executePreFlight(preFlightContent) {
    ...
    if (executeMatch) {
      const filePath = executeMatch[1].trim().replace('@.vibe-kit/', '.vibe-kit/');
